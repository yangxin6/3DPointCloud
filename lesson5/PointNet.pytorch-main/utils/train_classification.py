...
if __name__ == '__main__':
    ...
    opt.outf = opt.dataset_type + '/' + opt.outf
    print(opt)
    ...
    if opt.dataset_type == 'shapenet': #创建针对shapenet数据集的类对象
        ...
        test_dataset = ShapeNetDataset( #测试集
            ...
            split='test', #标记为测试
            npoints=opt.num_points,
            data_augmentation=False)  # 关闭关于 Z 轴的旋转
    ...
    # 可视化
    log_dir = os.path.join(opt.dataset_type, 'tensorboard', 'classification', 'train')
    train_writer = SummaryWriter(log_dir=log_dir)
    # 将 test accuracy 保存到 "tensorboard/test" 文件夹
    log_dir = os.path.join(opt.dataset_type, 'tensorboard', 'classification', 'test')
    ...
    for epoch in range(opt.nepoch): #在一个epoch下
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            # 绘制
            train_writer.add_scalar('Accuracy/train', correct.item() / float(opt.batchSize), epoch * num_batch + i)
            train_writer.add_scalar('Loss/train', loss.item(), epoch * num_batch + i)
            ...
            if i % 10 == 0: #每十次测试以下，过程同上
                ...
                correct = pred_choice.eq(target.data).cpu().sum()
                # 绘制
                test_writer.add_scalar('Accuracy/test', correct.item() / float(opt.batchSize), epoch * num_batch + i)
                test_writer.add_scalar('Loss/test', loss.item(), epoch * num_batch + i)
